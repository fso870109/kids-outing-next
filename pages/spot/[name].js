...
export async function getStaticProps({ params }) {
  ...
  // 同縣市其他景點推薦（排除自己，最多6個）
  const related = spot
    ? SPOTS.filter(s => s.city === spot.city && s.id !== spot.id).slice(0, 6)
    : [];
  return { props: { spot, related } };
}

export default function SpotPage({ spot, related }) {
  if (!spot) return <div style={{ padding:40, textAlign:"center" }}>找不到這個景點 <Link href="/">回首頁</Link></div>;
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://kids-outing.vercel.app/spot/${encodeURIComponent(spot.name)}`} />
      ...
      <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#fff8f0 0%,#f0f8ff 100%)", fontFamily:"'Noto Sans TC',sans-serif" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#ff6b6b,#ffa94d)", padding:"14px 16px", boxShadow:"0 4px 20px rgba(255,107,107,0.3)" }}>
          <Link href="/" style={{ color:"rgba(255,255,255,0.85)", textDecoration:"none", fontSize:13, display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
            ← 回首頁
          </Link>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:52, height:52, borderRadius:14, background:"rgba(255,255,255,0.25)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>{spot.emoji}</div>
            <div>
